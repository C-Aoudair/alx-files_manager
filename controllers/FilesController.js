...
class FilesController {
  ...
  static async getFile(req, res) {
    ...
    const fileData = FilesManager.getFile(file.localPath);
    if (fileDate == -1) return res.status(404).json({ error: "Not found" });

    return res.status(200).type(mime(file.name)).send(fileData.data);
  }
}
